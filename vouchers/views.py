...
def voucher_view(request):
    ...
    for voucher in vouchers:
        context["voucherrecord"].append(
            [voucher.voucher_display_id, voucher.voucher_reference_id, voucher.voucher_head, voucher.voucher_type,
             voucher.voucher_amount, voucher.voucher_date, voucher.remarks])
    context["expensevouchertypes"] = get_new_expense_voucher()
    context["incomevouchertypes"] = get_new_income_voucher()

    if request.method == "POST":
        if request.POST['btn'] in ["INCOME","EXPENSE"]:
        ...
        elif request.POST['btn'] == "new-record":
            name=request.POST["name"]
            type=request.POST["type"]
            acc=request.POST["acc"]
            VoucherTypes.objects.create(financial_year_serial=get_financial_year(),account_serial=acc,voucher_head=name,type=type,is_active=1,description='')
            return redirect('/vouchers/')
...
def get_new_expense_voucher():
    ...
    for type in types:
        vouchertypes.append(type.voucher_head)
    return vouchertypes
...
def printvoucher(request):
    if request.method == "GET":
        ...
        data["invoice"] = {
            ...
            "recs": [[rec.voucher_head, rec.voucher_amount]],
